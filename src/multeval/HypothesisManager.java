	...
	private void loadHyps(String[] hypFiles, String sys) throws IOException {
		if (numOptRuns == -1) {
			...
			if (numOptRuns != hypFiles.length) {
				throw new RuntimeException("Non-parallel number of optimization runs. Expected "
						+ numOptRuns + " but got " + hypFiles.length);
			}
		}
		...
		for (int iOpt = 0; iOpt < numOptRuns; iOpt++) {
			List<String> hypsForOptRun =
					loadSentences(hypFiles[iOpt], "Hypotheses for system " + sys + " opt run "
							+ (iOpt + 1));
			...
			if (numHyps == -1) {
				...
				if (numHyps != hypsForOptRun.size()) {
					throw new RuntimeException("Non-parallel inputs detected. Expected " + numHyps
							+ " hypotheses, but got " + hypsForOptRun.size());
				}
			}
		}
	}
