...
public class JBLEU {
	...
	public double score(int[] suffStats) {
		Preconditions.checkArgument(suffStats.length == N * 2 + 1,
				"BLEU sufficient stats must be of length N*2+1");
	...
	public static void main(String[] args) {
		...
		List<String> hyp =
				Lists.newArrayList(Splitter.on(' ').split("This is a hypothesis sentence ."));
		List<List<String>> refs = new ArrayList<List<String>>();
		refs.add(Lists.newArrayList(Splitter.on(' ')
				.split("This is the first reference sentence .")));
		refs.add(Lists.newArrayList(Splitter.on(' ').split(
